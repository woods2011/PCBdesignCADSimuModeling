...
public class ManualRouting : PcbDesignProcedure
{
    ...
    public ManualRouting(PcbDesignTechnology context) : base(context)
    {
        RequiredResources.AddRange(GetResourceRequestList());

        var pcbDescription = Context.PcbDescription;
        _remainTime = TimeSpan.FromHours(40)
                      * (pcbDescription.PinsCount / 300.0)
                      * (pcbDescription.PinDensity / 3.2)
                      * 0.08;
    }
    ...
    public override void InitResources()
    {
        ...
        _placementCpu.GoalPower = 0.6 * 4;
    }


    private List<IResourceRequest> GetResourceRequestList() => new()
    {
        new CpuRequest(CommonResReqId, 1, (0.6, 4.0)),
    ...
    public override string Name => "Интерактивная/Ручная Трассировка";
}